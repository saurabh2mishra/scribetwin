...
class MultiModalStyleModel:
    ...
    def __init__(self, config: Config, embedding_model: Optional[AuthorStyleModel] = None):
        self.config = config
    ...
    async def compute_similarity(
        ...
        if self.config.use_embedding_similarity:
            try:
                embedding_score = self.embedding_model.similarity_to_author(text, author_id)
